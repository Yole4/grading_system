        ...
        db.query(check, [studentInfo.LRN, "not"], (error, results) => {
            if (error) {
                ...
                if (results.length > 0) {
                    ...
                    const insertAccount = `INSERT INTO users (username, password, first_name, middle_name, last_name, user_type, added_by_id) VALUES (?,?,?,?,?,?,?)`;
                    db.query(insertAccount, [studentInfo.LRN, userHashedPassword, studentInfo.firstName, studentInfo.middleName, studentInfo.lastName, "Student", userId], (error, results) => {
                        if (error) {
                            res.status(401).json({ message: "Server side errors" });
                        } else {
                            // get the inserted id
                            const insertId = results.insertId;
                            const insert = `INSERT INTO user_info (user_id, lrn, first_name, middle_name, last_name, gender, civil_status, phone_number, religion, birth_place, address, date_of_birth, parent_guardian, curriculumn, teacher_id, grade, school_year, subject) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
                            db.query(insert, [insertId, studentInfo.LRN, studentInfo.firstName, studentInfo.middleName, studentInfo.lastName, studentInfo.gender, studentInfo.civilStatus, studentInfo.phoneNumber, studentInfo.religion, studentInfo.birthPlace, studentInfo.address, studentInfo.dateOfBirth, studentInfo.parentOrGuardian, studentInfo.curriculumn, studentInfo.teacher, studentInfo.grade, studentInfo.SY, studentInfo.subject], (error, results) => {
                                if (error) {
                                    res.status(401).json({ message: "Server side error!" });
                                } else {
                                    // notification here
...
// ###################################################################################################################################################################################
// ##################################################################### FETCH ALL USER INFORMATION BY ID  ###############################################################################
// ###################################################################################################################################################################################
app.post('/api/fetch-users-byID', verifyToken, (req, res) => {
    const {userId} = req.body;

    if (userId) {
        const select = `SELECT * FROM user_info WHERE isDelete = ? AND teacher_id = ?`;
        db.query(select, ["not", userId], (error, results) => {
            if (error) {
                res.status(401).json({ message: "Server side error!" });
            } else {
                res.status(200).json({ message: results });
...
// ###################################################################################################################################################################################
// ##################################################################### GET ALL STUDENT ACCOUNT EACH TEACHER  #######################################################################
// ###################################################################################################################################################################################
app.post('/api/fetch-all-student-account', verifyToken, (req, res) => {
    const {userId} = req.body;

    if (userId){
        const select = `SELECT * FROM users WHERE isDelete = ? AND added_by_id = ? AND user_type = ?`;
        db.query(select, ["not", userId, "Student"], (error, results) => {
            if (error) {
                res.status(401).json({message: "Server side error!"});
            }else{
                if (results.length > 0){
                    ...
                    res.status(401).json({message: "No student account found!"});
                }
            }
        });
    }else{
...
// ###################################################################################################################################################################################
// ##################################################################### ADD GRADE EACH TEACHER  #######################################################################
// ###################################################################################################################################################################################
app.post('/api/add-grade-byTeacher', verifyToken, (req, res) => {
    const {addGradeList, userId} = req.body;

    if (addGradeList, userId){
        // select if grade already exist each of teacher
        const select = `SELECT * FROM teacher_handle_grade WHERE grade = ? AND isDelete = ?`;
        db.query(select, [addGradeList, "not"], (error, results) => {
            if (error) {
                res.status(401).json({message: "server side error!"});
            }else{
                if (results.length > 0){
                    res.status(401).json({message: `Grade ${addGradeList} Already Exist!`})
                }else{
                    // insert new grade
                    const insert = `INSERT INTO teacher_handle_grade (teacher_id, grade) VALUES (?,?)`;
                    db.query(insert, [userId, addGradeList], (error, results) => {
                        if (error) {
                            res.status(401).json({message: "Server side error!"});
                        }else{
                            // insert notification
                            const insertNot = `INSERT INTO notificationS (user_id, notification_type, content) VALUES (?, ?, ?)`;
                            db.query(insertNot, [userId, "Add Grade", `You added new ${addGradeList} from the list`], (error, results) => {
                                if (error){
                                    res.status(401).json({message: "Server side error!"});
                                }else{
                                    res.status(200).json({message: `You've successfully added new grade ${addGradeList}`});
                                }
                            });
                        }
                    });
...
// ###################################################################################################################################################################################
// ##################################################################### FETCH GRADE EACH TEACHER  #######################################################################
// ###################################################################################################################################################################################
app.post('/api/get-each-grade', verifyToken, (req, res) => {
    const {userId} = req.body;

    if (userId){
        const select = `SELECT * FROM teacher_handle_grade WHERE teacher_id = ? AND isDelete = ?`;
        db.query(select, [userId, "not"], (error, results) => {
            if (error) {
                res.status(401).json({message: "Server side error!"});
            }else{
...
// ###################################################################################################################################################################################
// ##################################################################### UPDATE EACH GRADE LIST  #######################################################################
// ###################################################################################################################################################################################
app.post('/api/update-each-grade', verifyToken, (req, res) => {
    const {updateGradeList, updateId, teacher_id} = req.body;

    if (updateId, updateGradeList, teacher_id){
        const select = `SELECT * FROM teacher_handle_grade WHERE grade = ? AND teacher_id = ? AND id != ? AND isDelete = ?`;
        db.query(select, [updateGradeList, teacher_id, updateId, "not"], (error, results) => {
            if (error) {
                res.status(401).json({message: "server side error!"});
            }else{
                if (results.length > 0){
                    res.status(401).json({message: `Grade ${updateGradeList} is Already Exist!`});
                }else{
                    // update
                    const update = `UPDATE teacher_handle_grade SET grade = ? WHERE id = ?`;
                    db.query(update, [updateGradeList, updateId], (error, results) => {
                        if (error) {
                            res.status(401).json({message: "Server side error!"});
                        }else{
                            res.status(200).json({message: `Successfully updated to grade ${updateGradeList}`});
                        }
                    });
                }
            }
...
// ##################################################################### DELETE EACH GRADE  ############################################################################################
// ###################################################################################################################################################################################
app.post('/api/delete-each-grade', verifyToken, (req, res) => {
    const { updateId } = req.body;

    if (updateId) {
        // update
        const update = `UPDATE teacher_handle_grade SET isDelete = ? WHERE id = ?`;
        db.query(update, ["Deleted", updateId], (error, results) => {
            if (error) {
                res.status(401).json({ message: "Server side error!" });
            } else {
...
// ###################################################################################################################################################################################
// ##################################################################### ADD SUBJECT EACH TEACHER  #######################################################################
// ###################################################################################################################################################################################
app.post('/api/add-subject-byTeacher', verifyToken, (req, res) => {
    const {addSubject, userId} = req.body;

    if (addSubject, userId){
        // select if subject already exist each of teacher
        const select = `SELECT * FROM teacher_handle_subject WHERE subject = ? AND isDelete = ?`;
        db.query(select, [addSubject, "not"], (error, results) => {
            if (error) {
                res.status(401).json({message: "server side error!"});
            }else{
                if (results.length > 0){
                    res.status(401).json({message: `Grade ${addSubject} Already Exist!`})
                }else{
                    // insert new subject
                    const insert = `INSERT INTO teacher_handle_subject (teacher_id, subject) VALUES (?,?)`;
                    db.query(insert, [userId, addSubject], (error, results) => {
                        if (error) {
                            res.status(401).json({message: "Server side error!"});
                        }else{
                            // insert notification
                            const insertNot = `INSERT INTO notificationS (user_id, notification_type, content) VALUES (?, ?, ?)`;
                            db.query(insertNot, [userId, "Add Grade", `You added new ${addSubject} from the list`], (error, results) => {
                                if (error){
                                    res.status(401).json({message: "Server side error!"});
                                }else{
                                    res.status(200).json({message: `You've successfully added new subject ${addSubject}`});
                                }
                            });
                        }
                    });
...
// ###################################################################################################################################################################################
// ##################################################################### FETCH SUBJECT EACH TEACHER  #######################################################################
// ###################################################################################################################################################################################
app.post('/api/get-each-subject', verifyToken, (req, res) => {
    const {userId} = req.body;

    if (userId){
        const select = `SELECT * FROM teacher_handle_subject WHERE teacher_id = ? AND isDelete = ?`;
        db.query(select, [userId, "not"], (error, results) => {
            if (error) {
                res.status(401).json({message: "Server side error!"});
            }else{
                if (results.length > 0){
                    ...
                    res.status(401).json({message: "No subject found!"});
                }
            }
        });
    }else{
...
// ###################################################################################################################################################################################
// ##################################################################### UPDATE EACH SUBJECT LIST  #######################################################################
// ###################################################################################################################################################################################
app.post('/api/update-each-subject', verifyToken, (req, res) => {
    const {updateSubject, updateId, teacher_id} = req.body;

    if (updateId, updateSubject, teacher_id){
        const select = `SELECT * FROM teacher_handle_subject WHERE subject = ? AND teacher_id = ? AND id != ? AND isDelete = ?`;
        db.query(select, [updateSubject, teacher_id, updateId, "not"], (error, results) => {
            if (error) {
                res.status(401).json({message: "server side error!"});
            }else{
                if (results.length > 0){
                    res.status(401).json({message: `${updateSubject} is Already Exist!`});
                }else{
                    // update
                    const update = `UPDATE teacher_handle_subject SET subject = ? WHERE id = ?`;
                    db.query(update, [updateSubject, updateId], (error, results) => {
                        if (error) {
                            res.status(401).json({message: "Server side error!"});
                        }else{
                            res.status(200).json({message: `Successfully updated to ${updateSubject} subject`});
                        }
                    });
                }
            }
...
// ##################################################################### DELETE EACH SUBJECT  ############################################################################################
// ###################################################################################################################################################################################
app.post('/api/delete-each-subject', verifyToken, (req, res) => {
    const { updateId } = req.body;

    if (updateId) {
        // update
        const update = `UPDATE teacher_handle_subject SET isDelete = ? WHERE id = ?`;
        db.query(update, ["Deleted", updateId], (error, results) => {
            if (error) {
                res.status(401).json({ message: "Server side error!" });
            } else {
